...
#include "plot.h"
#include "pixmaps.h"
const QwtInterval radialInterval( 0.0, 200.0 );
...
Plot::Plot( QWidget *parent ):
    QwtPolarPlot( QwtText( "PPI Display" ), parent )
{
    setAutoReplot( true );//was false initially
    setPlotBackground( Qt::darkGreen );

    // scales
    setScale( QwtPolar::Azimuth,
        azimuthInterval.minValue(), azimuthInterval.maxValue(),
        azimuthInterval.width() / 12 );//12=Number of sections
    ...
    d_grid->setPen( QPen( Qt::darkBlue ) );
    ...
    marker->setPosition( QwtPointPolar( 60,100) );
    marker->setSymbol( new QwtSymbol( QwtSymbol::Star2,
        QBrush( Qt::white ), QPen( Qt::red ), QSize( 11,11 ) ) );
